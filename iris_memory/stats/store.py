...
class StatsKVStore:
    ...
    def __init__(self) -> None:
        self._get_kv: Callable[[str, Any], Awaitable[Any]] | None = None
        self._put_kv: Callable[[str, Any], Awaitable[None]] | None = None
    ...
    async def load_aggregated(self) -> dict[str, Any]:
        ...
        if not self._get_kv:
            logger.debug(
                "StatsKVStore: get_kv not configured, returning empty aggregated"
            )
            return {}
        ...
        try:
            ...
            logger.debug(
                f"StatsKVStore: Loaded aggregated stats, total_calls={data.get('total_calls', 0)}"
            )
            return data
    ...
    async def save_aggregated(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            self._last_aggregated = data
            logger.debug(
                f"StatsKVStore: Saved aggregated stats, total_calls={data.get('total_calls', 0)}"
            )
        except Exception as e:
