...
class LLMStatsRegistry:
    ...
    _instance: LLMStatsRegistry | None = None
    ...
    async def record_call(
        ...
        self._aggregated.calls_by_provider[pid] = (
            self._aggregated.calls_by_provider.get(pid, 0) + 1
        )
        self._aggregated.tokens_by_provider[pid] = (
            self._aggregated.tokens_by_provider.get(pid, 0) + tokens_used
        )
        self._aggregated.calls_by_source[source_module] = (
            self._aggregated.calls_by_source.get(source_module, 0) + 1
        )

        date_key = time.strftime("%Y-%m-%d")
        self._aggregated.calls_by_date[date_key] = (
            self._aggregated.calls_by_date.get(date_key, 0) + 1
        )
    ...
    def get_by_provider(self, provider_id: str) -> dict[str, Any]:
        ...
        records = [r for r in self._records if r.provider_id == provider_id]
    ...
    def get_by_source(self, source: str) -> dict[str, Any]:
        ...
        records = [r for r in self._records if r.source_module == source]
    ...
    def get_summary(self) -> StatsSummary:
        ...
        recent_errors = sum(1 for r in self._records[-100:] if not r.success)
        ...
        return StatsSummary(
            ...
            top_providers=[{"provider_id": p, "calls": c} for p, c in sorted_providers],
            top_sources=[{"source": s, "calls": c} for s, c in sorted_sources],
