...
CONTRADICTORY_RELATIONS: dict[KGRelationType, frozenset[KGRelationType]] = {
    ...
    KGRelationType.LIVES_IN: frozenset(
        {KGRelationType.WORKS_AT, KGRelationType.STUDIES_AT}
    ),
    ...
    async def detect_cycles(
        ...
        for start_node in candidate_nodes:
            if len(issues) >= max_issues:
                logger.debug(
                    f"Cycle detection: reached max_issues={max_issues}, stopping"
                )
                break

            # DFS 检测从 start_node 出发的短循环
            cycles = self._dfs_find_cycles(start_node, adj, max_cycle_length)
        ...
        logger.debug(
            f"Cycle detection: found {len(issues)} cycles (max_length={max_cycle_length})"
        )
        return issues
