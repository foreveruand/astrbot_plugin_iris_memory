...
CN_RELATION_PATTERNS: list[tuple[re.Pattern, KGRelationType, str]] = [
    ...
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:是|为)(?P<o>[\u4e00-\u9fa5A-Za-z]+)的(?:上司|领导|老板)"
        ),
        KGRelationType.BOSS_OF,
        "是...的上司",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:是|为)(?P<o>[\u4e00-\u9fa5A-Za-z]+)的(?:下属|手下)"
        ),
        KGRelationType.SUBORDINATE_OF,
        "是...的下属",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:和|与|跟)(?P<o>[\u4e00-\u9fa5A-Za-z]+)(?:是)?(?:朋友|好友|闺蜜|哥们)"
        ),
        KGRelationType.FRIEND_OF,
        "是朋友",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:和|与|跟)(?P<o>[\u4e00-\u9fa5A-Za-z]+)(?:是)?(?:同事|同学)"
        ),
        KGRelationType.COLLEAGUE_OF,
        "是同事",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:和|与|跟)(?P<o>[\u4e00-\u9fa5A-Za-z]+)(?:是)?(?:一家人|家人|亲戚|父母|兄弟|姐妹|夫妻|爸爸|妈妈|儿子|女儿)"
        ),
        KGRelationType.FAMILY_OF,
        "是家人",
    ),
    (
        re.compile(r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)认识(?P<o>[\u4e00-\u9fa5A-Za-z]+)"),
        KGRelationType.KNOWS,
        "认识",
    ),
    # 补充人际关系模式
    (
        re.compile(
            r"(?P<o>[\u4e00-\u9fa5A-Za-z]+)是(?P<s>[\u4e00-\u9fa5A-Za-z]+)的(?:朋友|好友|闺蜜|哥们|兄弟)"
        ),
        KGRelationType.FRIEND_OF,
        "是...的朋友",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:和|与|跟)(?P<o>[\u4e00-\u9fa5A-Za-z]+)(?:在)?(?:谈恋爱|交往|恋爱|约会)"
        ),
        KGRelationType.RELATED_TO,
        "谈恋爱",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)是(?P<o>[\u4e00-\u9fa5A-Za-z]+)的(?:导师|老师|师父|师傅|教练)"
        ),
        KGRelationType.RELATED_TO,
        "是...的导师",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)是(?P<o>[\u4e00-\u9fa5A-Za-z]+)的(?:学生|徒弟|弟子)"
        ),
        KGRelationType.RELATED_TO,
        "是...的学生",
    ),
    (
        re.compile(
            r"(?P<o>[\u4e00-\u9fa5A-Za-z]+)是(?P<s>[\u4e00-\u9fa5A-Za-z]+)的(?:家人|亲戚|父亲|母亲|哥哥|姐姐|弟弟|妹妹|老公|老婆|丈夫|妻子|爸爸|妈妈|儿子|女儿)"
        ),
        KGRelationType.FAMILY_OF,
        "是...的家人",
    ),
    (
        re.compile(
            r"(?P<o>[\u4e00-\u9fa5A-Za-z]+)是(?P<s>[\u4e00-\u9fa5A-Za-z]+)的(?:同事|同学|室友|邻居|搭档)"
        ),
        KGRelationType.COLLEAGUE_OF,
        "是...的同事",
    ),
    ...
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:住在|居住在|家在)(?P<o>[\u4e00-\u9fa5A-Za-z]+)"
        ),
    ...
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:在|就职于)(?P<o>[\u4e00-\u9fa5A-Za-z]+)(?:工作|上班|就职)"
        ),
    ...
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:在|就读于)(?P<o>[\u4e00-\u9fa5A-Za-z]+)(?:上学|读书|学习|就读)"
        ),
    ...
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:喜欢|爱|热爱|钟爱|迷恋)(?P<o>[\u4e00-\u9fa5A-Za-z]+)"
        ),
        KGRelationType.LIKES,
        "喜欢",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:讨厌|不喜欢|厌恶|反感)(?P<o>[\u4e00-\u9fa5A-Za-z]+)"
        ),
        KGRelationType.DISLIKES,
        "讨厌",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:有|拥有|养了)(?:一只|一个|一条)?(?P<o>[\u4e00-\u9fa5A-Za-z]+)"
        ),
        KGRelationType.HAS,
        "有",
    ),
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)(?:想要|想买|想学|想去)(?P<o>[\u4e00-\u9fa5A-Za-z]+)"
        ),
        KGRelationType.WANTS,
        "想要",
    ),
    # 身份
    (
        re.compile(
            r"(?P<s>[\u4e00-\u9fa5A-Za-z]+)是(?:一[名个位])?(?P<o>[\u4e00-\u9fa5A-Za-z]+(?:师|员|家|生|手|者))"
        ),
        KGRelationType.IS,
        "是",
    ),
...
EN_RELATION_PATTERNS: list[tuple[re.Pattern, KGRelationType, str]] = [
    (
        re.compile(
            r"(?P<s>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?) (?:is|was) (?:a )?(?:friend|buddy) of (?P<o>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)"
        ),
        KGRelationType.FRIEND_OF,
        "friend of",
    ),
    (
        re.compile(
            r"(?P<s>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?) (?:is|was) (?:the )?(?:boss|manager|supervisor) of (?P<o>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)"
        ),
        KGRelationType.BOSS_OF,
        "boss of",
    ),
    (
        re.compile(
            r"(?P<s>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?) (?:works|worked) at (?P<o>[A-Z][\w\s]+)"
        ),
        KGRelationType.WORKS_AT,
        "works at",
    ),
    (
        re.compile(
            r"(?P<s>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?) (?:lives|lived) in (?P<o>[A-Z][\w\s]+)"
        ),
        KGRelationType.LIVES_IN,
        "lives in",
    ),
    (
        re.compile(
            r"(?P<s>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?) (?:likes?|loves?) (?P<o>[\w\s]+)"
        ),
        KGRelationType.LIKES,
        "likes",
    ),
    (
        re.compile(
            r"(?P<s>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?) (?:hates?|dislikes?) (?P<o>[\w\s]+)"
        ),
        KGRelationType.DISLIKES,
        "dislikes",
    ),
