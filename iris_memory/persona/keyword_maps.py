...
class ExtractionResult:
    ...
    reply_style_preference: str | None = None
    formality_adjustment: float = 0.0
    topic_blacklist: list[str] = field(default_factory=list)
    work_info: str | None = None
    life_info: str | None = None
    ...
    # 工作维度
    work_style: str | None = None  # 工作风格：远程/坐班/自由
    work_challenge: str | None = None  # 工作挑战（追加到 work_challenges 列表）
    work_preferences: dict[str, Any] = field(default_factory=dict)  # 工作偏好

    # 生活维度
    lifestyle: str | None = None  # 生活方式：夜猫子/早起/宅
    ...
    def load(self) -> None:
        ...
        if self._yaml_path and self._yaml_path.exists():
            try:
                ...
                logger.warning(
                    f"Failed to load keyword maps from {self._yaml_path}: {e}"
                )
