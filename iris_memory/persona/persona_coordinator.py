...
class PersonaConflictDetector:
    ...
    def detect_conflicts(
        self, user_persona: dict[str, Any], bot_persona: str = "friendly"
    ) -> list[dict[str, Any]]:
        """检测人格冲突
    ...
    def _detect_style_conflicts(
        self, user_persona: dict[str, Any], bot_persona: str
    ) -> list[dict[str, Any]]:
        """检测风格冲突
    ...
    def get_resolution_suggestions(self, conflicts: list[dict[str, Any]]) -> list[str]:
        """获取冲突解决建议
        ...
        for conflict in conflicts:
            ...
            if conflict_type == ConflictType.STYLE_CONFLICT:
                suggestions.append(f"建议：{conflict['description']}")
...
class PersonaCoordinator:
    ...
    def coordinate_persona(
        ...
        if self.strategy == CoordinationStrategy.BOT_PRIORITY:
            ...
            return self._hybrid_prompt(
                bot_persona, user_persona, memory_context, conflicts
            )
        else:  # DYNAMIC
    ...
    def _bot_priority_prompt(self, bot_persona: str, memory_context: str) -> str:
        """Bot优先策略：Bot人格优先"""
        ...
        # 记忆上下文处理
        if memory_context:
            prompt_parts.append("同时参考用户画像中的偏好，但在风格上保持一致性")
    ...
    def _user_priority_prompt(
        ...
        # 记忆上下文
        if memory_context:
            prompt_parts.append("优先响应用户画像中的偏好，即使这与默认风格不同")
    ...
    def _hybrid_prompt(
        ...
        # 用户画像调整
        prompt_parts.append("同时根据用户画像调整回复细节和语气")
        ...
        trajectory = emotional_state.get("trajectory", "")
        if trajectory in ["deteriorating", "volatile"]:
            prompt_parts.append("当前用户情感状态不佳，适当调整回复风格，更加温和支持")

        # 记忆上下文
        if memory_context:
            prompt_parts.append("参考记忆上下文中的用户偏好，但不要被其限制")
