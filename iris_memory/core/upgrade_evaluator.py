...
请仅返回JSON，不要有其他文字。"""
    ...
    def _classify_evaluation_tier(
        self, memory: Memory, upgrade_type: str
    ) -> EvaluationTier:
        """将记忆分类到评估层级
        ...
        if upgrade_type == "working_to_episodic":
            if (
                memory.is_user_requested
                or (memory.access_count >= 5 and memory.importance_score > 0.6)
                or memory.emotional_weight > 0.8
                or memory.confidence >= 0.85
                or (memory.rif_score > 0.7 and memory.access_count >= 3)
                or _get_quality_level_value(memory.quality_level)
                >= QualityLevel.CONFIRMED.value
            ):
            ...
            if (
                ...
                and memory.emotional_weight <= 0.4
                and memory.confidence < 0.5
                and memory.rif_score <= 0.3
                and not memory.is_user_requested
                and _get_quality_level_value(memory.quality_level)
                < QualityLevel.HIGH_CONFIDENCE.value
            ):
            ...
            if (
                _get_quality_level_value(memory.quality_level)
                == QualityLevel.CONFIRMED.value
                or (memory.access_count >= 10 and memory.confidence > 0.7)
                or (memory.importance_score >= 0.9 and memory.access_count >= 5)
                or (memory.access_count >= 8 and memory.confidence > 0.8)
            ):
                return EvaluationTier.HIGH_CONFIDENCE_PASS

            if (
                memory.access_count < 2
                and memory.confidence <= 0.4
                and memory.importance_score < 0.5
            ):
                return EvaluationTier.CLEAR_REJECT
    ...
    def _rule_based_evaluation(
        self, memories: list[Memory], upgrade_type: str
    ) -> dict[str, tuple[bool, float, str]]:
        """基于规则的评估
        ...
        for memory in memories:
            if upgrade_type == "working_to_episodic":
                ...
                confidence = (
                    self._calculate_rule_confidence(memory, upgrade_type)
                    if should_upgrade
                    else 0.2
                )
            else:
    ...
    def _get_working_upgrade_reason(self, memory: Memory, should_upgrade: bool) -> str:
        ...
        if should_upgrade:
            if memory.is_user_requested:
                ...
                return (
                    f"访问{memory.access_count}次且重要性{memory.importance_score:.2f}"
                )
                ...
                ql_name = (
                    QualityLevel(ql_val).name
                    if ql_val <= QualityLevel.CONFIRMED.value
                    else str(ql_val)
                )
    ...
    async def _evaluate_batch_with_llm(
        ...
        if upgrade_type == "working_to_episodic":
            ...
            prompt = self.EPISODIC_TO_SEMANTIC_PROMPT.format(
        ...
        try:
            async with self._llm_semaphore:
                ...
                if not response or not response.strip():
                    logger.warning(
                        "LLM returned empty response (rate limit or error), falling back to rule-based evaluation"
                    )
                    return self._rule_based_evaluation(memories, upgrade_type)
    ...
    def _parse_llm_response(
        self, response: str, memories: list[Memory]
    ) -> dict[str, tuple[bool, float, str]]:
        """解析LLM响应
        ...
        try:
            ...
            for memory in memories:
                if memory.id not in results:
                    ...
                    should_upgrade = (
                        memory.should_upgrade_to_episodic()
                        if layer_val == "working"
                        else memory.should_upgrade_to_semantic()
                    )
                    results[memory.id] = (
                        should_upgrade,
                        0.5,
    ...
    async def _hybrid_evaluation(
        ...
        for memory in memories:
            ...
            if tier == EvaluationTier.HIGH_CONFIDENCE_PASS:
                ...
                reason = (
                    self._get_working_upgrade_reason(memory, True)
                    if upgrade_type == "working_to_episodic"
                    else self._get_episodic_upgrade_reason(memory, True)
                )
