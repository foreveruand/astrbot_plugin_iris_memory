...
_SCALAR_FIELDS: Sequence[_ScalarField] = (
    ...
    _ScalarField(
        "reply_style_preference", "preferred_reply_style", "interaction", "reply_style"
    ),
...
_DICT_MERGE_FIELDS: Sequence[_DictMergeField] = (
    ...
    _DictMergeField(
        "emotional_soothers", "emotional_soothers", "emotion", "emotion_soother"
    ),
    _DictMergeField(
        "social_boundaries", "social_boundaries", "relationship", "social_boundary", 0.9
    ),
)
...
_ADJUSTMENT_FIELDS: Sequence[_AdjustmentField] = (
    _AdjustmentField(
        "formality_adjustment", "communication_formality", "interaction", "formality"
    ),
    _AdjustmentField(
        "directness_adjustment", "communication_directness", "interaction", "directness"
    ),
    _AdjustmentField("humor_adjustment", "communication_humor", "interaction", "humor"),
    _AdjustmentField(
        "empathy_adjustment", "communication_empathy", "interaction", "empathy"
    ),
)

_LIST_ITERATE_FIELDS: Sequence[_ListIterateField] = (
    _ListIterateField(
        "topic_blacklist", "topic_blacklist", "interaction", "topic_blacklist"
    ),
    _ListIterateField(
        "emotional_triggers", "emotional_triggers", "emotion", "emotion_trigger"
    ),
)
    ...
    for fd in _SCALAR_FIELDS:
        ...
        if val:
            _apply(
                ...
                val,
                source_memory_id,
                memory_type or fd.mem_type,
                f"{rule_prefix}_{fd.rule_suffix}",
                conf * fd.conf_modifier,
                evidence,
            )
    ...
    for trait in _PERSONALITY_TRAITS:
        ...
        if delta != 0.0:
            ...
            _apply(
                ...
                new_val,
                source_memory_id,
                memory_type or "interaction",
                f"{rule_prefix}_personality_{trait}",
                conf * 0.7,
                evidence,
            )
