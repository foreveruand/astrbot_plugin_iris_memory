...
from iris_memory.core.constants import (
    NEGATIVE_EMOTION_STRINGS,
    NEGATIVE_EMOTIONS_ALL,
    NEGATIVE_EMOTIONS_CORE,
)
...
class EmotionContext:
    ...
    active_session: str | None = None  # 当前活动会话ID
    user_situation: str | None = None  # 用户当前处境
...
class EmotionalTrajectory:
    ...
    last_intervention: datetime | None = None  # 上次干预时间
...
class EmotionalState:
    ...
    history: deque[CurrentEmotionState] = field(
    ...
    def _analyze_trajectory(self):
        ...
        positive_count = sum(
            ...
            if e.primary in [EmotionType.JOY, EmotionType.EXCITEMENT, EmotionType.CALM]
        )
        negative_count = sum(
            1 for e in recent_emotions if e.primary in NEGATIVE_EMOTIONS_ALL
        )

        # 计算波动性
        intensity_values = [e.intensity for e in recent_emotions]
        avg_intensity = sum(intensity_values) / len(intensity_values)
        variance = sum((x - avg_intensity) ** 2 for x in intensity_values) / len(
            intensity_values
        )
        ...
        elif self.current.intensity > 0.8 and self.current.primary in [
            EmotionType.ANGER,
            EmotionType.ANXIETY,
        ]:
            self.trajectory.anomaly_detected = True
    ...
    def get_negative_ratio(self) -> float:
        ...
        negative_count = sum(
            count
            for emotion, count in self.patterns.items()
    ...
    def should_filter_positive(self) -> bool:
        ...
        """
        return (
            self.current.primary in NEGATIVE_EMOTIONS_CORE
            and self.current.intensity > 0.6
        )
        ...
        for key, value in self.__dict__.items():
            if isinstance(value, CurrentEmotionState):
                ...
                data[key] = {
                    ...
                    "last_intervention": value.last_intervention.isoformat()
                    if value.last_intervention
                    else None,
                }
                ...
                and value
                and isinstance(value[0], CurrentEmotionState)
            ):
                data[key] = [
                    {
                        "primary": e.primary.value,
    ...
    def from_dict(cls, data: dict[str, Any]) -> "EmotionalState":
        ...
        if "current" in data:
            ...
            data["current"] = CurrentEmotionState(
                ...
                detected_at=datetime.fromisoformat(
                    current_data.get("detected_at", datetime.now().isoformat())
                ),
        ...
        if "history" in data and data["history"]:
            data["history"] = [
                CurrentEmotionState(
                    ...
                    detected_at=datetime.fromisoformat(
                        e.get("detected_at", datetime.now().isoformat())
                    ),
        ...
        if "trajectory" in data:
            ...
            data["trajectory"] = EmotionalTrajectory(
                ...
                if traj_data.get("last_intervention")
                else None,
            )
