...
class MemoryCaptureEngine:
    ...
    def __init__(
        ...
        *,
        ...
        rif_threshold: float | None = None,
        enable_duplicate_check: bool | None = None,
        enable_conflict_check: bool | None = None,
        ...
        self.min_confidence = (
            ...
            else get_store().get("memory.min_confidence")
        )
        self.rif_threshold = (
            rif_threshold
            if rif_threshold is not None
            else get_store().get("memory.rif_threshold")
        )
        self.enable_duplicate_check = (
            enable_duplicate_check
            if enable_duplicate_check is not None
            else get_store().get("memory.enable_duplicate_check")
        )
        self.enable_conflict_check = (
            enable_conflict_check
            if enable_conflict_check is not None
            else get_store().get("memory.enable_conflict_check")
        )
        self.enable_entity_extraction = (
            enable_entity_extraction
            if enable_entity_extraction is not None
            else get_store().get("memory.enable_entity_extraction")
        )
    ...
    async def capture_memory(
        ...
        try:
            ...
            sensitivity_level, detected_entities = await self._detect_sensitivity(
                message, context, user_id
            )
            capture_log.sensitivity_detected(
                user_id,
                sensitivity_level.name
                if hasattr(sensitivity_level, "name")
                else str(sensitivity_level),
                detected_entities,
            )
            ...
            capture_log.emotion_analyzed(
                ...
                if hasattr(emotion_result["primary"], "value")
                else str(emotion_result["primary"]),
            ...
            requires_encryption = self.sensitivity_detector.get_encryption_required(
                sensitivity_level
            )
            ...
            capture_log.memory_created(
                ...
                memory_scope.value
                if hasattr(memory_scope, "value")
                else str(memory_scope),
            )
            ...
            capture_log.quality_assessed(
                user_id,
                memory.confidence,
                memory.quality_level.value
                if hasattr(memory.quality_level, "value")
                else str(memory.quality_level),
            )
            ...
            capture_log.storage_determined(
                user_id,
                memory.storage_layer.value
                if hasattr(memory.storage_layer, "value")
                else str(memory.storage_layer),
                "user_requested" if is_user_requested else "auto",
            )

            if self.chroma_manager and (
                self.enable_duplicate_check or self.enable_conflict_check
            ):
                ...
                if self.enable_duplicate_check and similar_memories:
                    duplicate = self.conflict_resolver.find_duplicate_from_results(
                ...
                if self.enable_conflict_check and similar_memories:
                    conflicts = self.conflict_resolver.find_conflicts_from_results(
                        memory, similar_memories
                    )
                    if conflicts:
                        should_store_new = (
                            await self.conflict_resolver.resolve_conflicts(
                                memory, conflicts, self.chroma_manager, self._kg_storage
                            )
                        )
                        capture_log.conflict_detected(
                            user_id, len(conflicts), should_store_new
                        )
    ...
    def _determine_memory_type(
        self, triggers: list[TriggerMatch], emotion_result: dict[str, Any]
    ) -> MemoryType:
        """确定记忆类型
    ...
    def _generate_summary(
        self, message: str, triggers: list[TriggerMatch]
    ) -> str | None:
        """生成记忆摘要
    ...
    def _assess_quality(
        ...
        consistency_score = self._calculate_consistency_score(
            memory, triggers, emotion_result
        )

        # 综合置信度（加权平均）
        memory.confidence = (
            trigger_conf * 0.4 + emotion_conf * 0.3 + consistency_score * 0.3
        )
        ...
        if not memory.is_user_requested:
            memory.importance_score = min(
                1.0, trigger_importance * 0.7 + emotion_importance
            )
        ...
        if triggers:
            ...
            # 偏好/事实触发器表示信息明确
            if any(
                t.type in [TriggerType.PREFERENCE, TriggerType.FACT] for t in triggers
            ):
    ...
    def _determine_storage_layer(self, memory: Memory, is_user_requested: bool):
        ...
        if memory.type == MemoryType.EMOTION and memory.subtype:
            ...
            if memory.emotion_decay_rate is None:
                memory.emotion_decay_rate = EmotionDecayProfile.get_decay_rate(
                    memory.subtype
                )
    ...
    def set_config(self, config: dict[str, Any]):
        ...
        self.rif_threshold = config.get(
            "rif_threshold", get_store().get("memory.rif_threshold")
        )
        self.enable_duplicate_check = config.get(
            "enable_duplicate_check", get_store().get("memory.enable_duplicate_check")
        )
        self.enable_conflict_check = config.get(
            "enable_conflict_check", get_store().get("memory.enable_conflict_check")
        )
        self.enable_entity_extraction = config.get(
            "enable_entity_extraction",
            get_store().get("memory.enable_entity_extraction"),
        )
    ...
    async def _detect_sensitivity(
        self, message: str, context: dict[str, Any] | None = None, user_id: str = ""
    ) -> tuple:
        """检测敏感度（支持LLM增强）
        ...
        if self._llm_sensitivity_detector:
            try:
                result = await self._llm_sensitivity_detector.detect(
