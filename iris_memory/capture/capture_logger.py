...
class CaptureLogger:
    ...
    def capture_start(
        self, user_id: str, message: str, is_user_requested: bool = False
    ) -> None:
    ...
    def negative_sample(self, user_id: str, reason: str = "chat") -> None:
        _logger.debug(f"CAPTURE.NEGATIVE_SAMPLE user={user_id} reason={reason}")

    def trigger_detected(self, user_id: str, triggers: list) -> None:
        if not triggers:
    ...
    def sensitivity_detected(
        self, user_id: str, level: str, entities: list[str]
    ) -> None:
        _logger.debug(
    ...
    def sensitivity_filtered(self, user_id: str, level: str) -> None:
        _logger.warning(f"CAPTURE.SENSITIVITY.FILTERED user={user_id} level={level}")

    def emotion_analyzed(
        self, user_id: str, primary: str, intensity: float, confidence: float
    ) -> None:
        _logger.debug(
    ...
    def memory_created(self, user_id: str, memory_type: str, scope: str) -> None:
        _logger.debug(
            f"CAPTURE.MEMORY.CREATE user={user_id} type={memory_type} scope={scope}"
        )

    def quality_assessed(
        self, user_id: str, confidence: float, quality_level: str
    ) -> None:
        _logger.debug(
    ...
    def rif_calculated(self, user_id: str, rif_score: float) -> None:
        _logger.debug(f"CAPTURE.RIF user={user_id} score={rif_score:.3f}")

    def storage_determined(self, user_id: str, storage_layer: str, reason: str) -> None:
        _logger.debug(
            f"CAPTURE.STORAGE user={user_id} layer={storage_layer} reason={reason}"
        )

    def duplicate_found(self, user_id: str, memory_id: str) -> None:
        _logger.info(f"CAPTURE.DUPLICATE user={user_id} dup_of={memory_id[:8]}...")

    def conflict_detected(
        self, user_id: str, conflict_count: int, resolved: bool
    ) -> None:
        _logger.debug(
    ...
    def capture_skip(self, user_id: str, reason: str) -> None:
        _logger.debug(f"CAPTURE.SKIP user={user_id} reason={reason}")

    def capture_error(self, user_id: str, error: Exception) -> None:
        _logger.error(f"CAPTURE.ERROR user={user_id} error={error}")
