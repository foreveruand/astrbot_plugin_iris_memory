...
class ContextBuilder:
    ...
    def _build_behavior_directives(
        ...
        directives = [
            "【记忆参考指南】",
            "◆ 背景吸收：注入的记忆只作为背景资料融入理解与回应，不要把它当作消息来源逐条转述。",
            "◆ 避免显式溯源：除非用户明确追问是谁说的、何时说的，或当前任务本身要求核对来源，否则不要使用“XXX说过”“你之前提到”“根据之前的记录”这类表述。优先直接给出自然、连贯的回答。",
            "◆ 谨慎引用：只有在来源本身对当前回答确实必要时，才简短说明信息来自过往对话；不要把记忆包装成证据链、会议纪要或检索结果。",
