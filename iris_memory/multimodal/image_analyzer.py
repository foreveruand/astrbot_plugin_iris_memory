...
用中文回复，不超过80字。"""
    ...
    def __init__(
        ...
        self.require_context_relevance = self.config.get(
        ...
        logger.debug(
            f"ImageAnalyzer initialized: enable={self.enable_analysis}, "
    ...
    async def analyze_message_images(
        ...
        umo: str = "",
        session_id: str = "",
        daily_analysis_budget: int | None = None,
    ...
    async def _analyze_single_image_with_checks(
        ...
        if level == ImageAnalysisLevel.SKIP:
            ...
            return ImageAnalysisResult(level=level, description="[图片]", token_cost=0)
        ...
        if not self._budget_manager.check_budget(user_id, session_id, daily_budget):
            ...
            return ImageAnalysisResult(
                level=ImageAnalysisLevel.SKIP, description="[图片]", token_cost=0
            )

        if self._similar_detector.is_similar(image_hash):
            logger.debug("Similar image detected recently, skipping analysis")
            self._stats["similar_skipped"] += 1
            return ImageAnalysisResult(
                level=ImageAnalysisLevel.SKIP, description="[图片]", token_cost=0
            )

        if self.require_context_relevance and not self._check_context_relevance(
            context_text, image_info
        ):
            logger.debug("Image not relevant to context, skipping detailed analysis")
        ...
        result = await self._analyze_single_image(
            image_info=image_info, level=level, context_text=context_text, umo=umo
        )
    ...
    def _extract_images(self, message_chain: list) -> list[ImageInfo]:
        ...
        for component in message_chain:
            ...
            if comp_type == "Image":
                ...
                is_sticker = self._is_sticker_url(url) or self._is_sticker_url(
                    file_path
                )

                images.append(ImageInfo(url=url, file=file_path, is_sticker=is_sticker))
    ...
    def _check_context_relevance(
        self, context_text: str, image_info: ImageInfo
    ) -> bool:
        """检查图片是否与上下文相关"""
    ...
    async def _analyze_single_image(
        self, image_info: ImageInfo, level: str, context_text: str, umo: str
    ) -> ImageAnalysisResult:
        """分析单张图片"""
        try:
            ...
            return ImageAnalysisResult(
                level=level, description=description, token_cost=token_cost
            )
            ...
            self._stats["errors"] += 1
            return ImageAnalysisResult(
                level=level, description="[图片]", token_cost=0, error=str(e)
            )
    ...
    async def _resolve_provider(self, umo: str = "") -> tuple:
        ...
        if provider_id and provider_id not in ("", "default"):
            try:
                ...
                logger.warning(
                    f"Provider not found: {provider_id}, falling back to default"
                )
            except Exception as e:
    ...
    def format_for_memory(
        self, results: list[ImageAnalysisResult], include_level: bool = False
    ) -> str:
        """将分析结果格式化为记忆存储格式"""
        if not results:
            return ""
        ...
        return {
            **self._stats,
            ...
            "cache_hit_rate": (
                ...
                / max(1, self._stats["total_analyzed"] + self._stats["cache_hits"])
            ),
    ...
    def _check_budget(
        ...
        """向后兼容方法：检查预算"""
        return self._budget_manager.check_budget(
            user_id, session_id, daily_analysis_budget
        )

    def _is_similar_recent_image(self, image_hash: str | None) -> bool:
        """向后兼容方法：检查相似图片"""
