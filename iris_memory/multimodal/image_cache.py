...
class ImageCacheManager:
    ...
    def get_image_hash(self, image_info: ImageInfo) -> str | None:
        """获取图片哈希用于缓存
    ...
    def _cleanup_cache(self) -> None:
        ...
        expired_keys = [
            k
            for k, (_, ts) in self._analysis_cache.items()
        ...
        if len(self._analysis_cache) >= self.max_cache_size:
            sorted_items = sorted(self._analysis_cache.items(), key=lambda x: x[1][1])
...
class ImageBudgetManager:
    ...
    def __init__(
        self, daily_budget: int = 100, session_budget: int = 20, cooldown: float = 3.0
    ):
        """初始化预算管理器
    ...
    def check_budget(
        ...
        user_id: str,
        session_id: str = "",
        daily_budget_override: int | None = None,
    ) -> bool:
        """检查是否超出分析预算
        ...
        if daily_count >= budget_limit:
            logger.debug(
                f"Daily analysis budget exhausted: {daily_count}/{budget_limit}"
            )
            return False
        ...
        if session_id and self.session_budget > 0:
            ...
            if session_count >= self.session_budget:
                logger.debug(
                    f"Session analysis budget exhausted: {session_count}/{self.session_budget}"
                )
                return False
...
class SimilarImageDetector:
    ...
    def __init__(self, time_window: int = 60, limit: int = 20):
        """初始化检测器
    ...
    def is_similar(self, image_hash: str | None) -> bool:
        """检查是否为最近分析过的相似图片
