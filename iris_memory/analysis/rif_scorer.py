...
class RIFScorer:
    ...
    def calculate_rif(
        self, memory: Memory, context: dict[str, Any] | None = None
    ) -> float:
        """计算记忆的RIF评分
        ...
        try:
            ...
            logger.debug(
                f"Traditional RIF calculated for memory {memory.id[:8]}: "
    ...
    def _calculate_recency(self, memory: Memory) -> float:
        ...
        if memory.type == MemoryType.EMOTION and memory.subtype:
            ...
            emotion_time_score = EmotionDecayProfile.calculate_emotion_time_score(
                memory
            )
