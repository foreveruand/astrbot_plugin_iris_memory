...
class EmotionAnalyzer:
    ...
    def _init_emotion_dict(self):
        ...
        # 程度副词（不应被当作否定词）
        self.degree_words = ["非常", "很", "十分", "极其", "特别", "相当"]
    ...
    def _init_rules(self):
        ...
        self.rules = [
            ...
            {"pattern": r"[？?]{2,}$", "emotion": EmotionType.ANXIETY, "weight": 0.6},
            # 标点规则
            {"pattern": r"[~～]{3,}", "emotion": EmotionType.JOY, "weight": 0.5},
            # 上下文规则（讽刺检测）
            {
    ...
    async def analyze_emotion(
        ...
        primary, secondary, intensity, confidence = self._combine_results(
            dict_result, rule_result, contextual_correction
        )
    ...
    def _analyze_by_dict(self, text: str) -> dict[str, Any]:
        """使用情感词典分析（30%权重）"""
        emotion_scores = dict.fromkeys(EmotionType, 0)
        ...
        for neg in self.negation_words:
            ...
            if neg in text:
                # 检查是否在程度副词中
                is_part_of_degree = any(
                    degree_word in text and neg in degree_word
                    for degree_word in self.degree_words
                )
        ...
        if has_negation:
            ...
            if joy_score > 0:
                ...
                emotion_scores[EmotionType.NEUTRAL] = (
                    emotion_scores.get(EmotionType.NEUTRAL, 0) + joy_score * 0.5
                )

            if excitement_score > 0:
                emotion_scores[EmotionType.EXCITEMENT] = 0
                emotion_scores[EmotionType.CALM] = (
                    emotion_scores.get(EmotionType.CALM, 0) + excitement_score * 0.5
                )
        ...
        # 找到次要情感（得分>0）
        secondary = [
            e for e, score in emotion_scores.items() if score > 0 and e != primary
        ]
    ...
    def _detect_contextual_correction(
        ...
        # 检测讽刺
        sarcastic_patterns = [r"真是.*呀", r"你真行", r"太棒了（？！）", r"呵呵"]
        ...
        if "history" in context and context["history"]:
            ...
            negative_count = sum(
                1 for e in recent_emotions if e in NEGATIVE_EMOTIONS_CORE
            )
    ...
    def _combine_results(
        ...
        for sec in dict_result["secondary"]:
            emotion_scores[sec] = (
                emotion_scores.get(sec, 0)
                + dict_result["intensity"] * 0.1 * dict_weight
            )

        # 规则结果
        if rule_result["confidence"] > 0:
            rule_primary = rule_result["primary"]
            emotion_scores[rule_primary] = (
                emotion_scores.get(rule_primary, 0)
                + rule_result["intensity"] * rule_weight
            )
        ...
        # 找次要情感
        secondary = [
            e for e, score in emotion_scores.items() if e != primary and score > 0.1
        ][:2]
    ...
    def analyze_time_series(self, emotional_state: EmotionalState) -> dict[str, Any]:
        """分析情感时序
