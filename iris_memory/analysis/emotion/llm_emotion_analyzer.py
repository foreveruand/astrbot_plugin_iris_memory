...
_COMPLEX_INDICATORS = ["又", "但", "不过", "虽然", "尽管", "矛盾", "纠结", "复杂"]
...
class LLMEmotionAnalyzer(LLMEnhancedDetector[EmotionAnalysisResult]):
    ...
    def _parse_llm_result(self, data: dict[str, Any]) -> EmotionAnalysisResult:
        ...
        return EmotionAnalysisResult(
            ...
            intensity=BaseDetectionResult.clamp(float(data.get("intensity", 0.5))),
    ...
    def _might_need_context_awareness(self, text: str) -> bool:
        ...
        return any(
            ind in text_lower for ind in _SARCASM_INDICATORS + _COMPLEX_INDICATORS
        )

    def _merge_results(
        self, rule_result: EmotionAnalysisResult, llm_result: EmotionAnalysisResult
    ) -> EmotionAnalysisResult:
        """合并规则和LLM结果"""
        # 讽刺/复杂情感 → 直接使用LLM结果
