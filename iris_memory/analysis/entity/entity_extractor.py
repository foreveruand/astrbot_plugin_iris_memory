...
class EntityExtractor:
    ...
    def _compile_patterns(self):
        ...
        chinese_name_pattern = (
            f"[{''.join(self.chinese_surnames)}][\\u4e00-\\u9fa5]{{1,2}}"
        )
        ...
        self.patterns = {
            ...
            EntityType.TIME: [re.compile(time_relative_pattern, re.IGNORECASE)],
            EntityType.DATE: [
            ...
            EntityType.EMAIL: [re.compile(email_pattern)],
            EntityType.URL: [re.compile(url_pattern)],
            EntityType.MONEY: [
    ...
    def _normalize_entity(self, entity: Entity):
        ...
        if entity.entity_type == EntityType.TIME:
            ...
            entity.metadata["normalized_quantity"] = self._normalize_quantity(
                entity.text
            )
    ...
    def _normalize_time(self, text: str) -> datetime | None:
        """标准化时间实体
        ...
        try:
            ...
            hour_match = re.search(
                r"(\d{1,2})(?::(\d{2}))?\s*(?:am|pm|上午|下午|晚上|早上|中午|夜里|凌晨)?",
                text,
            )
            if hour_match:
                ...
                if (
                    ...
                    return datetime(
                        base_date.year, base_date.month, base_date.day, 20, 0
                    )
                elif "下午" in text or "afternoon" in text:
                    return datetime(
                        base_date.year, base_date.month, base_date.day, 14, 0
                    )
                elif "早上" in text or "今早" in text or "morning" in text:
                    return datetime(
                        base_date.year, base_date.month, base_date.day, 9, 0
                    )
                elif "中午" in text:
                    return datetime(
                    ...
                    return datetime(
                        base_date.year, base_date.month, base_date.day, 10, 0
                    )
    ...
    def _normalize_date(self, text: str) -> datetime | None:
        """标准化日期实体
        ...
        try:
            ...
            match = re.match(
                r"(\w{3})\w*\s+(\d{1,2})(?:st|nd|rd|th)?,?\s*(\d{4})",
                text,
                re.IGNORECASE,
            )
            if match:
    ...
    def _normalize_money(self, text: str) -> float | None:
        """标准化金额实体
    ...
    def _normalize_quantity(self, text: str) -> dict[str, Any] | None:
        """标准化数量实体
        ...
        try:
            match = re.match(
                r"([\d.]+)\s*([个只条件本台次遍趟公斤kg千克克升升L毫升ml米m厘米cm毫米mmpiecespcsitems]+)",
                text,
                re.IGNORECASE,
            )
            if match:
