...
class LLMMessageProcessor:
    ...
    async def generate_summary(
        self, messages: list[str], user_id: str, context: dict | None = None
...
分析："""
    ...
    def _build_summary_prompt(
        self, messages: list[str], context: dict | None = None
    ) -> str:
        """构建摘要提示词"""
        formatted_messages = "\n".join(
...
总结："""
    ...
    def _circuit_breaker_on_failure(self) -> None:
        ...
        if self._cb_state == "half_open":
            self._cb_state = "open"
            logger.warning("LLM circuit breaker OPEN (half_open probe failed)")
