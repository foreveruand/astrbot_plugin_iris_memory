...
class CacheEntry:
    ...
    access_count: int = 0
    ttl: int | None = None  # 生存时间（秒），None表示永不过期
    ...
    def get(self, key: str) -> Any | None:
        ...
        if old_count == self._min_freq:
            ...
            remaining = [
                ...
                if v.access_count == old_count and v.key != key
            ]
            if not remaining:
                # 提升最小频率
...
class WorkingMemoryCache:
    ...
    async def add_memory(
        self, user_id: str, group_id: str | None, memory_id: str, memory: Any
    ) -> bool:
        """添加记忆到工作记忆缓存
        ...
        async with self._lock:
            ...
            if session_key not in self._sessions:
                ...
                self._sessions[session_key] = LRUCache(
                    self.max_memories_per_session, self.ttl
                )
    ...
    async def clear_session(self, user_id: str, group_id: str | None = None) -> bool:
        ...
        async with self._lock:
            if group_id is None:
                # 清空用户的所有会话
                keys_to_delete = [
                    k for k in self._sessions.keys() if k.startswith(f"{user_id}:")
                ]
...
class CacheContentCompressor:
    ...
    def compress_memory(self, content: str, entities: list[Any] | None = None) -> str:
        ...
        # 拆分句子
        sentences = [
            s.strip() for s in self._SENTENCE_DELIMITERS.split(content) if s.strip()
        ]
        if not sentences:
        ...
        if entities:
            for ent in entities:
                text = getattr(ent, "text", None) or (
                    ent if isinstance(ent, str) else ""
                )
                if text:
...
class CacheManager:
    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        return {"embedding_cache": embedding_stats, "working_cache": working_stats}
