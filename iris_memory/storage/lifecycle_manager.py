...
class SessionLifecycleManager:
    ...
    def __init__(
        ...
        # 升级评估器
        self.upgrade_mode = (
            UpgradeMode(upgrade_mode) if isinstance(upgrade_mode, str) else upgrade_mode
        )
        ...
        self.promotion_task = None  # Deprecated: now handled by scheduled tasks
        self.extraction_task = None  # Deprecated: now handled by scheduled tasks
        self.is_running = False

        # 语义提取器（延迟初始化）
        self._semantic_extractor = None
        self._semantic_extraction_enabled = get_store().get(
            "semantic_extraction.enabled"
        )
        self._semantic_extraction_interval = get_store().get(
        ...
        self._grace_period_manager: GracePeriodManager | None = None
        self._grace_period_enabled: bool = get_store().get(
            "memory.grace_period.enable", True
        )
    ...
    def set_llm_provider(self, llm_provider):
        ...
        logger.debug(
            f"LLM provider set for upgrade evaluation (mode={self.upgrade_mode.value})"
        )
    ...
    async def start(self):
        ...
        # Only start cleanup loop (non-LLM task)
        # Memory promotion and semantic extraction are now handled by AstrBot's cron_manager
        self.cleanup_task = asyncio.create_task(self._cleanup_loop())

        # Initialize semantic extractor if enabled (for cron to call later)
    ...
    async def stop(self):
        ...
        if self.cleanup_task:
            ...
            try:
                ...
                logger.warning(f"[Hot-Reload] Error cancelling cleanup task: {e}")
            self.cleanup_task = None

        # promotion_task and extraction_task are now handled by cron_manager
    ...
    def _init_semantic_extractor(self) -> None:
        ...
        clustering = SemanticClustering(
            ...
            cluster_time_window_days=store.get(
            ...
            max_memories_per_cluster=store.get(
    ...
    async def _promote_memories(self):
        ...
        try:
            ...
            remove_failures: list[
                str
            ] = []  # 已写入Chroma但未能从工作记忆移除的memory_id

            # 收集所有会话中的工作记忆及其会话上下文
            all_working_memories = []
            memory_session_map: dict[
                str, tuple
            ...
            for session_key, session_data in all_sessions.items():
                ...
                group_id = (
                    parts[1] if len(parts) > 1 and parts[1] != "private" else None
                )
                ...
                for memory in working_memories:
                    ...
                    if memory.storage_layer == StorageLayer.EPISODIC:
                        logger.debug(
                            f"Skipping memory {memory.id}: already EPISODIC (likely leftover from failed remove)"
                        )
                        continue
            ...
            if all_working_memories:
                evaluation_results = (
                    await self.upgrade_evaluator.evaluate_working_to_episodic(
                        all_working_memories
                    )
                )
                ...
                for memory in all_working_memories:
                    ...
                    try:
                        ...
                        if success:
                            ...
                            logger.warning(
                                f"Failed to add memory {memory.id} to Chroma"
                            )
                    except Exception as e:
                        failed_promotions += 1
                        memory.storage_layer = original_layer
            ...
            try:
                episodic_memories = (
                    await self.chroma_manager.get_memories_by_storage_layer(
            ...
            if episodic_memories:
                logger.debug(
                    f"Evaluating {len(episodic_memories)} episodic memories for SEMANTIC upgrade"
                )

                # 使用升级评估器进行判断
                evaluation_results = (
                    await self.upgrade_evaluator.evaluate_episodic_to_semantic(
                        episodic_memories
                    )
                )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to get semantic memories for demotion check: {e}"
                )
                semantic_memories = []
            ...
            if session_state == SessionState.ACTIVE:
                ...
                if inactive_duration > self.session_timeout:
                    ...
                    if await self._archive_session(session_key):
                        self._update_session_state(session_key, SessionState.ARCHIVED)
                        archived_count += 1
                    else:
                        # 无法归档，直接关闭
        ...
        if self._grace_period_manager and self.chroma_manager:
            try:
                ...
                for memory in pending:
                    if (
                        memory.grace_period_expires_at
                        and memory.grace_period_expires_at <= now
                    ):
                        try:
                            ...
                            logger.warning(
                                f"Failed to delete expired grace period memory {memory.id}: {e}"
                            )
                if expired_count > 0:
                    logger.debug(
                        f"Grace period cleanup: {expired_count} expired memories deleted"
                    )
            except Exception as e:
                logger.warning(f"Grace period cleanup error: {e}")

    def _update_session_state(self, session_key: str, new_state: SessionState):
        """更新会话状态
        ...
        try:
            ...
            for memory in working_memories:
                ...
                if result and result[0]:  # should_upgrade = True
                    ...
                    gp_result = await self._grace_period_manager.evaluate_and_apply(
                        memory
                    )
            ...
            if all_persist and self.chroma_manager:
                for memory in all_persist:
                    try:
                        ...
                        logger.debug(f"Memory {memory.id} added to Chroma as EPISODIC")
                    except Exception as e:
                        logger.error(f"Error persisting memory {memory.id}: {e}")
    ...
    async def deactivate_session(self, user_id: str, group_id: str | None = None):
        """停用会话（转为非活跃）
    ...
    async def close_session(self, user_id: str, group_id: str | None = None):
        """关闭会话
    ...
    async def deserialize_state(self, data: dict[str, Any]):
        ...
        for session_key, state_data in data.items():
            try:
                ...
                # 如果状态无效，使用默认值
                logger.warning(
                    f"Invalid state '{state_data.get('state')}' for session {session_key}, using default 'inactive'"
                )
            ...
            try:
                ...
                # 如果日期无效，使用当前时间
                logger.warning(
                    f"Invalid last_active for session {session_key}, using current time"
                )
                last_active = datetime.now()
            ...
            try:
                ...
                # 如果日期无效，使用当前时间
                logger.warning(
                    f"Invalid last_updated for session {session_key}, using current time"
                )
                last_updated = datetime.now()
