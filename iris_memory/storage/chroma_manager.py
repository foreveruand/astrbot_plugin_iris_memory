...
class ChromaManager:
    ...
    def __init__(self, config, data_path: Path, plugin_context=None):
        ...
        self.embedding_model_name = cfg.get(
            "embedding.local_model", "BAAI/bge-small-zh-v1.5"
        )
        ...
        self.reimport_on_dimension_conflict = cfg.get(
            "embedding.reimport_on_dimension_conflict", True
        )
        ...
        self._queries: ChromaQueries | None = None
        self._operations: ChromaOperations | None = None

    @property
    def is_ready(self) -> bool:
        """检查 Chroma 管理器是否已初始化并可用"""
        return (
            self._is_ready and self.client is not None and self.collection is not None
        )
    ...
    async def _generate_embedding(self, text: str) -> list[float] | None:
        ...
        try:
            embedding = await self.embedding_manager.embed(
    ...
    def _result_to_memory(self, memory_data: dict[str, Any]) -> Memory:
        ...
        if evidence_ids_str:
            memory.evidence_ids = [
                eid.strip() for eid in evidence_ids_str.split(",") if eid.strip()
            ]
        if "last_validated" in metadata and metadata["last_validated"]:
            try:
                memory.last_validated = datetime.fromisoformat(
    ...
    def _set_memory_timestamps(self, memory, metadata: dict) -> None:
        ...
        if "last_access_time" in metadata:
            try:
                memory.last_access_time = datetime.fromisoformat(
    ...
    async def get_all_memories(
        ...
        try:
            self._ensure_ready()
            return await self._queries.get_all_memories(
    ...
    async def count_memories(
        ...
        try:
            self._ensure_ready()
            return await self._queries.count_memories(
                user_id, group_id, storage_layer, persona_id
            )
        except (RuntimeError, StorageNotReadyError):
            return 0
    ...
    async def get_memories_by_storage_layer(
        ...
        try:
            self._ensure_ready()
            return await self._queries.get_memories_by_storage_layer(
                storage_layer, limit
            )
        except (RuntimeError, StorageNotReadyError):
            return []
