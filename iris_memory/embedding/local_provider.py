...
class LocalProvider(EmbeddingProvider):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"LocalProvider initialized, background model loading started: {model_name}"
            )
            return True
    ...
    def _start_background_load(self, model_name: str):
        ...
        def _load_model():
            try:
                ...
                logger.debug(
                    f"[Background] Loading local embedding model: {model_name} on {self.device}"
                )
    ...
    def _wait_for_model(self, timeout: float = 120) -> None:
        ...
        if self._load_complete.is_set():
            if self._load_error:
                raise RuntimeError(
                    f"Local embedding model failed to load: {self._load_error}"
                )
            return
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        status = {
            ...
            "status": "loading"
            if loading
            else ("ok" if self._model_instance else "error"),
            "model": self._model,
