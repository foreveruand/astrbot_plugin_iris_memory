...
class TokenBudget:
    ...
    def __init__(
        self, total_budget: int = 512, preamble_cost: int = 20, postamble_cost: int = 10
    ):
        """初始化Token预算
    ...
    def can_add_memory(self, memory_text: str, as_summary: bool = True) -> bool:
        """判断是否可以添加记忆
...
class MemoryCompressor:
    ...
    def compress_memories(
        self, memories: list[tuple[str, str | None]]
    ) -> list[tuple[str, bool]]:
        """批量压缩记忆
...
class DynamicMemorySelector:
    ...
    def __init__(
        self, token_budget: TokenBudget, compressor: MemoryCompressor | None = None
    ):
        """初始化选择器
    ...
    def select_memories(self, memories: list, target_count: int) -> tuple[list, dict]:
        """选择要注入的记忆
        ...
        sorted_memories = sorted(
            memories, key=lambda m: m.rif_score * m.importance_score, reverse=True
        )
    ...
    def _format_roleplay(
        ...
        lines = ["【你的记忆】"]
        lines.append(
            "这些都是你亲身经历的事情，回复时可以自然地说'我记得...'、'你之前说过...'："
        )
