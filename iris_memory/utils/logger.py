...
def setup_logging(
    ...
    if log_dir is None:
        try:
            ...
            log_dir = (
                Path(get_astrbot_data_path()) / "plugin_data" / "iris_memory" / "logs"
            )
        except ImportError:
...
def _configure_logger(logger: logging.Logger, name: str) -> None:
    ...
    formatter = logging.Formatter(
        _LOG_CONFIG["format"], datefmt=_LOG_CONFIG["date_format"]
    )
...
def init_logging_from_config(config: Any, plugin_data_path: Path) -> None:
    ...
    setup_logging(
        log_dir=log_dir, level=level, max_bytes=max_bytes, backup_count=backup_count
    )
...
def _cleanup_old_logs(log_dir: Path) -> None:
    ...
    try:
        ...
        log_files = [
            f
            for f in log_dir.glob("iris_memory.log.*")
